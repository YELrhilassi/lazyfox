...
// `node build.ts --dev` (npm run build) defines __DEV__=true so the dev-only
...
writeFileSync(
  ...
  "// GENERATED by build.ts from core/js/core.wasm \u2014 do not edit.\n" +
...
// host step degrade cleanly — so this step never blocks the extension build.
{
  ...
  try {
    ...
    console.warn("[native-host] build failed (host is optional; skipping): " + String(e instanceof Error && e.message ? e.message : e));
  }
}
...
//
// Release mode (RELEASE=1, used by the master workflow): sync-signed-xpi.ts
...
//
// Normal mode: amo-sign.ts reuses the committed signed artifact offline, or
...
if (process.env.RELEASE === "1") {
  ...
  const sync = spawnSync(process.execPath, [join(root, "scripts", "sync-signed-xpi.ts")], { stdio: "inherit" });
  if (sync.status !== 0) {
    // sync-signed-xpi.ts already printed the reason. Fail gently instead of
  ...
  serverRun(process.execPath, [join(root, "scripts", "amo-sign.ts")]);
}

...
# Delegate to the unified Go TUI installer (recommended; one cross-platform
# binary with an interactive wizard). Falls back to the legacy logic below if
# no binary or Go toolchain is available.
$lzBin = $null
foreach ($c in @(
...
if ($lzBin) {
  ...
  if ($NoExtension) { $goArgs += '-NoExtension' }
  if ($NoLaunch) { $goArgs += '-NoLaunch' }
  if ($ChromeLoaderOnly) { $goArgs += '-ChromeLoaderOnly' }

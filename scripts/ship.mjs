...
import { loadEnv } from "./amo-lib.mjs";
...
function shout(cmd, args = [], opts = {}) {
  ...
  try {
    ...
    fail(`command failed (${cmd} ${args.join(" ")}): ${e.status ?? e.message}`);
  }
}
const gh = (args) => {
  try {
...
const { api } = await import("./amo-lib.mjs");
...
shout("node", [path.join(__dirname, "check-dist.mjs")]);
shout("node", [path.join(__dirname, "test-installer.mjs")]);
const { isSignedXpi, xpiVersion } = await import("./amo-lib.mjs");
{
  try {
    ...
    fail(`could not verify the signed xpi: ${e.message}`);
  }
}
